<?php
 $con=mysqli_connect("localhost","root","","fourwheels"); 
	...
	if(isset($_POST["email"])) {
		$email=$_POST["email"];
		$senha=$_POST["senha"];
	$login="select*from volfisico where email='$email' and senha='$senha'";
	...
	$info=mysqli_fetch_assoc($aces);
		if(empty($info)){
			...
			$_SESSION['fisi']=$info["idfisi"];
			header("location: homevf.php");		
		}	
	}
	...
	<div class="jumbotron jumbotron-fluid bg-with">
	  <div class="container">
		  <center>
			...
			<form name="loguser" method="POST" action="lgvf.php">
			  <div class="form-row">
